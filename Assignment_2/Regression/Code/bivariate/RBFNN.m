...
M_values = [2,5,10,25,50];  % no of basis functions
...
for i = 1:length(M_values)
    ...
    for j = 1:length(lambda_values)
        ...
        [coeffs,designMat,centroids,widthParam,phi_tilda] = surface_fit(train_data(:,1:2),train_data(:,3),length(train_data),M,lambda);
        model_output_train = designMat*coeffs;
        target_output_train = train_data(:,3);
        ...
        for i1=1:size(val_data,1)
            for j1=1:M
                designMat_val(i1,j1) = exp((-norm(val_data(i1,1:2)-centroids(j1,:))^2)/widthParam);
            end
        end
        model_output_val = designMat_val*coeffs;
        target_output_val = val_data(:,3);
        ...
        for i1=1:size(test_data,1)
            for j1=1:M
                designMat_test(i1,j1) = exp((-norm(test_data(i1,1:2)-centroids(j1,:))^2)/widthParam);
            end
        end
        model_output_test = designMat_test*coeffs;
        target_output_test = test_data(:,3);
...
[coeffs,designMat,centroids,widthParam,phi_tilda] = surface_fit(train_data(:,1:2),train_data(:,3),length(train_data),best_M,best_lambda);
model_output_train = designMat*coeffs;
figure, plot3(train_data(:,1), train_data(:,2), train_data(:,3), 'r*',train_data(:,1), train_data(:,2), model_output_train,'b*')
...
model_output_val = designMat_val*coeffs;
figure, plot3(val_data(:,1), val_data(:,2), val_data(:,3),'r*',val_data(:,1),val_data(:,2),model_output_val,'b*')
...
model_output_test = designMat_test*coeffs;
figure, plot3(test_data(:,1), test_data(:,2),test_data(:,3),'r*', test_data(:,1), test_data(:,2),model_output_test,'b*')
...
figure, scatter(model_output_train, train_data(:,3),'ko','filled'); 
axis equal;
ylabel('Target output');
xlabel('Model output');
title('Scatter Plot for training data');

figure, scatter(model_output_val, val_data(:,3),'ko','filled'); 
axis equal;
ylabel('Target output');
xlabel('Model output');
title('Scatter Plot for validation data');

figure, scatter(model_output_test, test_data(:,3),'ko','filled'); 
axis equal;
ylabel('Target output');
xlabel('Model output');
